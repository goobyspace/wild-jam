...
func attack():
	get_checkers()
	if wait_for_attack:
		await get_tree().create_timer(startup * repeat).timeout
		await get_tree().create_timer(active_time * repeat).timeout
		await get_tree().create_timer(cleanup_time * repeat).timeout
	return true
